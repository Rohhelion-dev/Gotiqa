...
const metrics = [
  {
    label: 'Total Herd',
    value: '127',
    subtitle: <span className="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">+12 this month</span>,
  },
  { label: 'Monthly Revenue', value: 'KSh 540,000', subtitle: 'From sales & milk production' },
  {
    label: 'Vaccinated',
    value: '98%',
    subtitle: <span className="text-xs font-medium text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full">All animals healthy</span>,
  },
  { label: 'Farm Staff', value: '4', subtitle: '2 full-time workers' },
];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {metrics.map((m) => (
          <StatCard key={m.label} label={m.label} value={m.value} subtitle={m.subtitle} />
        ))}
      </div>
